...
const TruckEx = models.truckprofile;
...
passport.use(
  ...
  new JwtexStrategy(
    ...
    async (payload, done) => {
      try {
        //find user specified in token
        const user = await TruckEx.findById(payload.sub);
...
passport.use(
  ...
  new LocalexStrategy(
    ...
    async (email, password, done) => {
      try {
        const user = await TruckEx.findOne({ email });
        // If not, handle it
        if (!user) {
          return done(null, false);
        }
...
passport.use(
  ...
  new JwtexStrategy(
    ...
    async (payload, done) => {
      try {
        //find user specified in token
        const user = await Crop.findById(payload.sub);
...
passport.use(
  ...
  new LocalexStrategy(
    ...
    async (email, password, done) => {
      try {
        const user = await Crop.findOne({ email });
        // If not, handle it
        if (!user) {
          return done(null, false);
        }
...
passport.use(
  ...
  new JwtexStrategy(
    ...
    async (payload, done) => {
      try {
        //find user specified in token
        const user = await NgoInquiry.findById(payload.sub);
...
passport.use(
  ...
  new LocalexStrategy(
    ...
    async (email, password, done) => {
      try {
        const user = await NgoInquiry.findOne({ email });
        // If not, handle it
        if (!user) {
          return done(null, false);
        }
