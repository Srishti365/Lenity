...
router
  ...
  .post(
    validateBody(confirmPassword.authSchema),
...
router
  .route("/editProfile")
  .post(
    validateBody(editProfile.authSchema),
    passportJWT,
...
router
  .route("/othersProfile/:name")
  .get(passportJWT, UsersController.othersProfile);

router.route("/deleteProfile").post(passportJWT, UsersController.deleteProfile);

router.get("/chat", passportJWT, async (req, res) => {
  try {
    let id = req.user.id;
    console.log(id);
    var pos = null;
      ...
      for (var i = 0; i < result.length; i++) {
        for (var j = 0; j < result[i].itemRequested.length; j++) {
          if (
            (result[i].itemRequested[j].confirmationStatus === 1 ||
              result[i].itemRequested[j].confirmationStatus === 2) &&
            result[i].requestor === req.user.local.username &&
            result[i].itemRequested[j].status === false
          ) {
            // result[i].itemRequested.splice(j,1);
...
router.post(
  ...
  async (req, res, next) => {
    try {
      var str = req.body.stringVar;
      var arr = str.split("|");
      console.log("-----------------a--r-ray", arr);
      var id = arr[0];
      var user = arr[1];
