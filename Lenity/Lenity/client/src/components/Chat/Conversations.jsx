...
const Conversations = props => {
    ...
    const handleRecipient = recipients => {
        console.log("abcd",recipients);
        for (let i = 0; i < recipients.length; i++) {
            if ( 
                recipients[i].local.username !==
                authenticationService.currentUserValue.username
            ) {console.log("recepients::::",recipients[i]);
                return recipients[i];
            }
        }
        return null;
                    ...
                    {conversations.map(c => (
                        <ListItem
                            ...
                            onClick={() => {
                                ...
                                props.setScope(
                                    handleRecipient(c.recipientObj).local.username
                                );
                            }}
                        >
                            <ListItemAvatar>
                            ...
                            <ListItemText
                                primary={handleRecipient(c.recipientObj).local.username}
                                secondary={
                                    <React.Fragment>
                                        {c.lastMessage}
                                    </React.Fragment>
