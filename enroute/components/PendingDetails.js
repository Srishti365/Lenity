...
export default class PendingDetails extends Component {
  ...
  viewRoute = async (event, value) => {
    ...
    //console.log("button pressed", value);
    var response = await axios.post(
      "http://10.0.55.37:8080/delivery/view_deliveryroute",
      {
        id: this.state.details1._id
      },
    ...
    this.setState(
      {
        startLat: response.data.lat_farm,
        startLong: response.data.long_farm,
        endLat: response.data.lat_storage,
        endLong: response.data.long_storage
      },
      () => {
        console.log(
          "latlongs: ",
